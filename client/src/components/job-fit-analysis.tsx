...
export function JobFitAnalysis({ fitData, onCopySuggestion }: JobFitAnalysisProps) {
      ...
      <CardContent>
        {fitData.jd_title ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-foreground">Role Match Score</h3>
                <span 
                  className={`text-2xl font-bold ${getMatchColor(fitData.match_score)}`}
              ...
              <div className="w-full bg-muted rounded-full h-3 mb-6">
                <div 
                  className={`h-3 rounded-full ${
                    fitData.match_score >= 80 ? "bg-primary" : 
                    fitData.match_score >= 60 ? "bg-accent" : "bg-destructive"
                  }`}
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="text-sm font-semibold text-foreground mb-2">Target Role</h4>
                  <p className="text-muted-foreground" data-testid="target-role">
                    {fitData.jd_title}
                  </p>
                </div>
                
                <div>
                  <h4 className="text-sm font-semibold text-foreground mb-2">Match Strength</h4>
                  <p className="text-muted-foreground">
                    {getMatchText(fitData.match_score)} - {
                      fitData.match_score >= 80 ? "Strong alignment with role requirements" :
                      fitData.match_score >= 60 ? "Good alignment with most requirements" :
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">No Job Description Provided</h3>
            <p className="text-muted-foreground mb-6">
              Add a job description during analysis to get role-specific fit analysis and recommendations
            </p>
